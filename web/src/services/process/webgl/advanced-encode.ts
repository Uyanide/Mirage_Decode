import type {
  PrismAdvancedEncodeConfig,
  PrismAdvancedEncodeInputConfig,
  PrismAdvancedEncodeService,
} from '../../../services/prism-advanced-encode';
...
// just a normal implementation, no need to use WebGL for this
export function WebGLAdvancedEncodeProcess<TBase extends WebGLProcessConstructor>(Base: TBase) {
  return class extends Base implements PrismAdvancedEncodeService {
    prismAdvancedEncode({ inputs, output }: PrismAdvancedEncodeConfig): boolean {
      if (inputs.length === 0) {
