...
export type FallbackProcessConstructor = new (...args: any[]) => FallbackCommonProcess;

export class FallbackCommonProcess implements ImageCommonService {
  private static toGrayLum = (r: number, g: number, b: number): number => {
    return r * 0.299 + g * 0.587 + b * 0.114;
  };

  private static toGrayAverage = (r: number, g: number, b: number): number => {
    return (r + g + b) / 3;
  };

  toGray(src: Ptr<ImageData>, tar: Ptr<ImageData>, algo: ToGrayAlgo = 'Lum') {
    if (!src.v) {
      tar.v = null;
      return;
    }
    ...
    const toGrayPixel = algo === 'Lum' ? FallbackCommonProcess.toGrayLum : FallbackCommonProcess.toGrayAverage;
    const tarData = tar.v.data;
    for (let i = 0; i < src.v.data.length; i += 4) {
      const r = srcData[i];
      const g = srcData[i + 1];
      const b = srcData[i + 2];
      const gray = toGrayPixel(r, g, b);
      tarData[i] = gray;
      tarData[i + 1] = gray;
      tarData[i + 2] = gray;
  ...
  adjustContrast(contrast: number, src: Ptr<ImageData>, tar: Ptr<ImageData>) {
    if (!src.v) {
      tar.v = null;
      return;
    }
  ...
  resizeCover(width: number, height: number, src: Ptr<ImageData>, tar: Ptr<ImageData>) {
  ...
  resizeFit(maxSize: number, src: Ptr<ImageData>, tar: Ptr<ImageData>) {
    if (!src.v || maxSize <= 0) {
      tar.v = null;
      return;
    }
